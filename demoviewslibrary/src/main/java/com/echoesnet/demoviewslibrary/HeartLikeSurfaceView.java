...
public class HeartLikeSurfaceView extends SurfaceView implements SurfaceHolder.Callback {
    public static final int[] DRAWABLE_IDS = new int[]{
            ...
            R.drawable.heart_5, R.drawable.heart_6, R.drawable.praise, R.drawable.heart_8,
...
//        pagerCanvas.drawRect(new RectF(0, 0, viewW, viewH), paint);
    ...
    private DrawObject createDrawObj(int i,int resid){
        ...
        Point pStart = new Point(viewW/2,viewH);
...
//        L.eSysInfo("HeartLikeSurface","mark: "+mark," c1x: "+c1x);
            ...
            while (isRunning) {
                synchronized (mHolder) {
                    try {
                        ...
                        if(null!=imgKey){ drawList.add(createDrawObj(mRandom.nextInt(2),imgKey));}
...
//                        L.eSysInfo("HeartLikeSurface","相差时间 "+t," 上次 "+lastTimePoint);
...
//                                    float threshold_ratio = 0.2f; //缩放阈值
//                                    float threshold_MaxRatio = 1.0f; //缩放阈值
...
//                                    Log.d("ratio","->"+ratio);
                                    ...
                                    Bitmap bm = LruCacheBitmapLoader.getInstance().getResBitmap(getContext(),dobj.getImgKey());
...
//                                    dobj.setDirection(angle);
                                    ...
                                    willRemoveObj.add(i);
//                                    L.eSysInfo("HeartLikeSurface","将被移除 index: "+i," path长度 "+desLen + " 所处位置 "+sumRunLen+" Data index "+i +" 速度："+speed);
                                }
                            }
                        ...
                        if(drawList.isEmpty()){ //检测没有元素在canvas中，停止刷新
                            isRunning = false;
                        }
...
//                        L.eSysInfo("HeartLikeSurface","可以延迟的时间 "+youCanDelayTime," 工作时间 "+lastTimePoint);
